    ...
    <div className="min-h-screen">
      ...
      {loading ? (
        ...
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {research.map(r => {
              const meta = typeMeta[r.research_type] || { icon: FileText, bar: 'bg-[#23CE6B]', chip: 'bg-[#4F6D7A]/10 text-[#23CE6B] border-[#4F6D7A]/20' };
              ...
              return (
                <Card key={r.id} className="border-[#4F6D7A]/20 transition-all hover:shadow-xl hover:shadow-[#23CE6B]/25 hover:-translate-y-1 cursor-pointer overflow-hidden group"
                  ...
                  <CardContent className="p-6">
                    ...
                    <h3 className="font-semibold text-sm leading-snug mb-2 line-clamp-2 group-hover:text-[#243010] transition-colors">{r.title}</h3>
